# Multi-stage build for Playwright Report Server
...
COPY --from=builder --chown=nodejs:nodejs /app/apps/playwright-server/dist ./dist
...
RUN mkdir -p /app/reports && chown nodejs:nodejs /app/reports
